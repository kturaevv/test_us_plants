...
from src.auth.database import auth_user_t, refresh_tokens_t
...
async def get_user_by_id(user_id: int) -> dict[str, Any] | None:
    select_query = select(auth_user_t).where(auth_user_t.c.id == user_id)

    return await fetch_one(select_query)


async def get_user_by_email(email: str) -> dict[str, Any] | None:
    select_query = select(auth_user_t).where(auth_user_t.c.email == email)
...
async def create_refresh_token(
    *, user_id: int, refresh_token: str | None = None
    ...
    insert_query = refresh_tokens_t.insert().values(
        uuid=uuid.uuid4(),
...
async def get_refresh_token(refresh_token: str) -> dict[str, Any] | None:
    select_query = refresh_tokens_t.select().where(
        refresh_tokens_t.c.refresh_token == refresh_token
    )
...
async def expire_refresh_token(refresh_token_uuid: UUID4) -> None:
    update_query = (
        refresh_tokens_t.update()
        .values(expires_at=datetime.utcnow() - timedelta(days=1))
        .where(refresh_tokens_t.c.uuid == refresh_token_uuid)
    )
